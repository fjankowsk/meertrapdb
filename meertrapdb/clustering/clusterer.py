...
class Clusterer(object):
    ...
    def __repr__(self):
        ...
        info_dict = {"time_thresh": self.time_thresh, "dm_thresh": self.dm_thresh}
    ...
    def match_candidates(self, t_candidates):
        ...
        for i in range(len(candidates)):
            ...
            if info["processed"][mask_cand]:
                self.__log.debug(
                    "Candidate was already assigned a cluster, skipping it: {0}".format(
                        cand["index"]
                    )
                )
                continue
        ...
        if len(candidates) > 0:
            ...
            for field in ["members", "beams"]:
                self.__log.info(
                    "{0} (min, mean, median, max): {1}, {2}, {3}, {4}".format(
                        field.capitalize(),
                        np.min(info[field]),
                        np.mean(info[field]),
